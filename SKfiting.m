function f = SKfiting(X, Y, Vhat, fSettings)
...
% load all settings from fSettings file
A = importdata(fSettings);
gammaP = A(1,1);         % select default correlation function
fmincon_algor = A(2,1);
fmincon_maxEval = A(3,1);
...
display('Stochastic Kriging started.');
fprintf('Number of sample points: %d\n', size(X,1));
fprintf('Stochastic Kriging will now run with the following settings:\n');
fprintf('\tCorrelation function: %s\n', corr_func);
fprintf('\tOptimization algorithm: %s\n', algor);
fprintf('\tMaximum number of evaluation: %d\n', fmincon_maxEval);
display('.............................................');
...
myopt = optimset('Display','final-detailed','MaxFunEvals',fmincon_maxEval,'MaxIter',1000,...
    'Algorithm', algor);
...
fprintf('Stochastic Kriging finished.\n');
display('---------------------------------------------------------------');
